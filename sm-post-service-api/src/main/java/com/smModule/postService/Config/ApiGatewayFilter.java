...
public class ApiGatewayFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        ...
        String gatewayHeader = servletRequest.getHeader(PostConstants.GATEWAY_HEADER);
        if (!Objects.equals(gatewayHeader, PostConstants.GATEWAY_SECRET)){
            servletResponse.setStatus(HttpStatus.UNAUTHORIZED.value());
            response.setContentType(JSON_TYPE);
            OutputStream output = response.getOutputStream();
            ObjectMapper mapper = new ObjectMapper();
            mapper.writeValue(output, new ApiResponse(PostConstants.DIRECT_CALL_VALIDATION_RESPONSE, false));
            output.flush();
        }
